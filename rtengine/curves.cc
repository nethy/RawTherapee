...
void PerceptualToneCurve::BatchApply(const size_t start, const size_t end, float *rc, float *gc, float *bc, const PerceptualToneCurveState &state) const
{
    ...
    for (size_t i = start; i < end; ++i) {
        ...
        // panchromatic lunimance coefficients
        constexpr float Yr = 0.3333333f;
        constexpr float Yg = 0.3333333f;
        constexpr float Yb = 0.3333333f;

        // original color luminance
        const float lr = CurveFactory::igamma2(r / 65535.f);
        const float lg = CurveFactory::igamma2(g / 65535.f);
        const float lb = CurveFactory::igamma2(b / 65535.f);
        float currentLuminance = CurveFactory::gamma2(lr * Yr + lg * Yg + lb * Yb) * 65535.f;
        currentLuminance = currentLuminance == 0.f ? 0.00001f : currentLuminance;

        const float targetLuminance = lutToneCurve[currentLuminance];

        // color luminance of film-like tone curve
        const float lar = CurveFactory::igamma2(ar / 65535.f);
        const float lag = CurveFactory::igamma2(ag / 65535.f);
        const float lab = CurveFactory::igamma2(ab / 65535.f);
        float adobeLuminance = CurveFactory::gamma2(lar * Yr + lag * Yg + lab * Yb) * 65535.f;
        adobeLuminance = adobeLuminance == 0.f ? 0.00001f : adobeLuminance;

        // adjust adobe tone curve luminance to match target luminance
        const float Acoef = targetLuminance / adobeLuminance;
        ar = CLIP<float>(ar * Acoef);
        ag = CLIP<float>(ag * Acoef);
        ab = CLIP<float>(ab * Acoef);

        // luminance tone curve
        const float Lcoef = targetLuminance / currentLuminance;
        r = CLIP<float>(r * Lcoef);
        g = CLIP<float>(g * Lcoef);
        b = CLIP<float>(b * Lcoef);

        // interpolate the two tone curves
        const float weight = 0.5f;
        r = CLIP<float>(intp(weight, ar, r));
        g = CLIP<float>(intp(weight, ag, g));
        b = CLIP<float>(intp(weight, ab, b));

        setUnlessOOG(rc[i], gc[i], bc[i], r, g, b);
    }
}
